...
class PolygonUtils:
  ...
  def fetch_image(self, maptype, params, marker=None):
    ...
    if marker:
      params += "&markers=color:%s|size:tiny|%s" % ( # adding params to url
        self.dot_color,
    ...
    return io.imread(self.base_url + params) # returns samples/staticmap in pixel format


  def center_image(self, maptype, point, zoom, marker=None):
    params = "center=%s&size=%s&zoom=%i" % (
      self.point_to_string(point), # (coordinates)
      self.tile_to_string(self.tile_size), # 500x500, instance variable
      zoom # 20, passed in
    )
